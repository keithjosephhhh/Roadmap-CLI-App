...
class RoadmapTracker:
    ...
    def view_tasks(self, days=7, status=None):
    ...
    def view_by_category(self, category, status=None):
    ...
    def add_progress_note(self, note):
        ...
        self.conn.commit()
        self.console.print(f"[green]Progress note added![/green]")
...
def tasks(days, status):
    """View upcoming tasks"""
    tracker = RoadmapTracker()
    tracker.view_tasks(days, status)

@cli.command()
@click.argument('category')
@click.option('--status', default=None, help='Filter tasks by status (e.g., pending, completed)')
def by_category(category, status):
    """View tasks by category"""
    tracker = RoadmapTracker()
    tracker.view_by_category(category, status)
...
def categories():
    ...
    console = Console()
    console.print(Panel("\n".join([f"- {cat}" for cat in cats]), title="Available Categories", style="bold cyan"))
...
def welcome():
    ...
    console = Console()
    console.print(Panel(Text("Welcome to the Learning Roadmap Tracker!\nUse the CLI to manage your tasks and progress.", justify="center"), style="bold blue"))
